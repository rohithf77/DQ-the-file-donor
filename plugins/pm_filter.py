...
async def next_page(bot, query):
    ...
    try:
        if settings['max_btn']:
            ...
            if n_offset == 0:
                ...
                btn.append([InlineKeyboardButton("𝗣𝗔𝗚𝗘", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/10)+1} / {math.ceil(total/10)}", callback_data="pages"), InlineKeyboardButton("𝐍𝐄𝐗𝐓 ➪", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
            ...
            if n_offset == 0:
                ...
                btn.append([InlineKeyboardButton("𝗣𝗔𝗚𝗘", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/int(MAX_B_TN))+1} / {math.ceil(total/int(MAX_B_TN))}", callback_data="pages"), InlineKeyboardButton("𝐍𝐄𝐗𝐓 ➪", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
                        InlineKeyboardButton("👈🏼𝗕𝗔𝗖𝗞", callback_data=f"next_{req}_{key}_{off_set}"),
    ...
    btn.insert(0, [
        InlineKeyboardButton("𝗖𝗛𝗘𝗖𝗞 𝗕𝗢𝗧 𝗣𝗠 𝗕𝗥𝗨𝗛", url=f"https://t.me/{temp.U_NAME}")
    ])
    try:
...
async def language_check(bot, query):
    ...
    if files:
        ...
        if offset != "":
            ...
            btn.append(
                [InlineKeyboardButton(text="NO MORE PAGES AVAILABLE BRUH",callback_data="pages")]
            )
        try:
            await query.edit_message_reply_markup(
                reply_markup=InlineKeyboardMarkup(btn)
...
async def select_language(bot, query):
    ...
    btn = [[
        ...
        InlineKeyboardButton("𝗘𝗡𝗚𝗟𝗜𝗦𝗛", callback_data=f"lang#{userid}#eng"),
        InlineKeyboardButton("𝗧𝗔𝗠𝗜𝗟", callback_data=f"lang#{userid}#tam"),
        InlineKeyboardButton("𝗛𝗜𝗡𝗗𝗜", callback_data=f"lang#{userid}#hin")
    ],[
        InlineKeyboardButton("𝗞𝗔𝗡𝗡𝗔𝗗𝗔", callback_data=f"lang#{userid}#kan"),
        InlineKeyboardButton("𝗧𝗘𝗟𝗨𝗚𝗨", callback_data=f"lang#{userid}#tel")
    ],[
        InlineKeyboardButton("𝗠𝗔𝗟𝗔𝗬𝗔𝗟𝗔𝗠", callback_data=f"lang#{userid}#mal")
    ],[
        InlineKeyboardButton("𝗠𝗨𝗟𝗧𝗜 𝗔𝗨𝗗𝗜𝗢", callback_data=f"lang#{userid}#multi"),
        InlineKeyboardButton("𝗗𝗨𝗔𝗟 𝗔𝗨𝗗𝗜𝗢", callback_data=f"lang#{userid}#dual")
    ],[
                    ...
                    if IS_VERIFY and not await check_verification(client, query.from_user.id):
                        ...
                        await client.send_cached_media(
                            ...
                            reply_markup=InlineKeyboardMarkup(
                                [
                                [
                                InlineKeyboardButton("➲ Gʀᴏᴜᴩ", url="https://t.me/+K6DIefAu3SpmMDll")
                                ]
                                ]
                            )
                        )
...
async def auto_filter(client, msg, spoll=False):
    ...
    btn.insert(0, [
        InlineKeyboardButton("CHECK BOT PM BRUH", url=f"https://t.me/{temp.U_NAME}")
    ])
